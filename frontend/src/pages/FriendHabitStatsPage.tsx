...
import { getFriendHabitStats } from "../api";
...
export function FriendHabitStatsPage() {
  const { t } = useTranslation();
  const { friendId, habitId } = useParams();
  const [stats, setStats] = useState<HabitStatsResponse | null>(null);
  const [error, setError] = useState<string | null>(null);

  useEffect(() => {
    if (!friendId || !habitId) return;
    getFriendHabitStats(friendId, habitId).then(setStats).catch((e: Error) => setError(e.message));
  }, [friendId, habitId]);
  ...
  return (
    <section className="space-y-3 pb-8">
      <div className="glass-card p-4">
        <h2 className="text-lg font-black text-ink">{t("friendHabitStats")}</h2>
